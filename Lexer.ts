import { commands, command, argument, parsedCommand } from "./commands.ts";
import { variablesNames } from "./variables.ts";
...
class Lexer {
  ...
  parseText(text: string): parsedCommand | null {
    ...
    if (this.isCommandSupported(command)) {
      let parsedArgs = this.parseArgs(command, args);
      if (parsedArgs !== null) {
        parsedCommand = {
          name: command,
    ...
    if (len !== commandObj?.args) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (curArgType !== parsedArgs[i].type) {
          logger.error(
            `command ${command} doesn't support type '${curArg}' as argument number ${
              i + 1
            }`
          );
          parsedArgs = null;
  ...
  parseArguments(args: string[]): argument[] {
    ...
    for (let i = 0, len = args.length; i < len; i++) {
      ...
      if (!isNaN(Number(curArg))) {
        ...
        type = "boolean";
        value = curArg === "true" ? true : false;
      } else if (variablesNames.includes(curArg)) {
        type = "var";
      } else {
        type = "unknown";
      }
